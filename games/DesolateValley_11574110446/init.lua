--[[ Desolate Valley — PlaceId 11574110446 ]]
	...
	local function fetch(u)
		...
		if root and typeof(readfile) == "function" then
			...
			if string.sub(nu, 1, #nr) == nr then
				...
				error("Desolate Valley: readfile failed: " .. u)
			end
		end
		return game:HttpGet(u, true)
	end

	_G.MYA_REPO_BASE = normalizeBase(ctx.baseUrl)
	_G.MYA_FETCH = fetch

	local runSrc = fetch(base .. "runtime.lua")
	local runChunk = loadstring(runSrc, "@DesolateValley/runtime")
	if typeof(runChunk) ~= "function" then
		error("Desolate Valley: runtime.lua failed to compile")
	end
	runChunk()

	local guiSrc = fetch(base .. "gui.lua")
	local guiChunk = loadstring(guiSrc, "@DesolateValley/gui")
	if typeof(guiChunk) ~= "function" then
		error("Desolate Valley: gui.lua failed to compile")
	end
	guiChunk()

	ctx.notify("Desolate Valley module loaded")
end
