--[[ Project Delta — rose / plum GUI ]]
local P = _G.MYA_PROJECT_DELTA
if not P then
	error("[MyaProjectDelta] Load runtime.lua first")
end
...
local CONFIG_FOLDER = "mya_project_delta_configs"
...
if typeof(fetch) ~= "function" or typeof(repoBase) ~= "string" or #repoBase == 0 then
	error("[MyaProjectDelta] MYA_FETCH / MYA_REPO_BASE missing — mount via hub with ctx.baseUrl, or set init globals.")
end
local libSrc = fetch(repoBase .. "lib/mya_game_ui.lua")
if typeof(libSrc) ~= "string" or #libSrc == 0 then
	error("[MyaProjectDelta] Could not load lib/mya_game_ui.lua from repo base: " .. repoBase)
end
local libFn = loadstring(libSrc, "@lib/mya_game_ui")
if typeof(libFn) ~= "function" then
	error("[MyaProjectDelta] lib/mya_game_ui.lua failed to compile")
end
local MyaUI = libFn()
...
_G.mya_project_delta_notif_ui = notif_ui
...
local visuals_players = all_sub_pages["Visuals"]["Players"]
...
local ref_aim_npcs = make_toggle_row(combat_aim, "Target NPCs", next_order(), P.get_aim_assist_npcs, P.set_aim_assist_npcs)
...
lo = 0
section_label(visuals_players, "Players", next_order())
local ref_esp = make_toggle_row(visuals_players, "ESP highlights", next_order(), P.get_esp, P.set_esp)
local ref_esp_team = make_toggle_row(visuals_players, "Team check", next_order(), P.get_esp_team_check, P.set_esp_team_check)
...
local ref_esp_view_angle = make_toggle_row(visuals_players, "View angle (facing arrow)", next_order(), P.get_esp_view_angle, P.set_esp_view_angle)
local ref_health = make_toggle_row(visuals_players, "Health bars", next_order(), P.get_healthbars, P.set_healthbars)
local ref_esp_dist = make_toggle_row(visuals_players, "Distance text", next_order(), P.get_esp_distance, P.set_esp_distance)
local ref_esp_names = make_toggle_row(visuals_players, "Player names", next_order(), P.get_esp_names, P.set_esp_names)

lo = 0
section_label(visuals_world, "World ESP", next_order())
local ref_npc_esp = make_toggle_row(visuals_world, "NPC ESP", next_order(), P.get_npc_esp, P.set_npc_esp)
local ref_crate_esp = make_toggle_row(visuals_world, "Crates ESP", next_order(), P.get_crate_esp, P.set_crate_esp)
local set_crate_dist_slider = make_slider(visuals_world, "Crate max distance", next_order(), 20, 2000, P.get_crate_esp_max_dist(), "%.0f", function(v)
	P.set_crate_esp_max_dist(v)
end)
local set_world_dist_slider = make_slider(visuals_world, "World max distance (NPC, vehicle, mines, extract)", next_order(), 100, 8000, P.get_world_esp_max_dist(), "%.0f", function(v)
	P.set_world_esp_max_dist(v)
end)
local ref_extraction_esp = make_toggle_row(visuals_world, "Extraction ESP", next_order(), P.get_extraction_esp, P.set_extraction_esp)
local ref_landmine_esp = make_toggle_row(visuals_world, "Landmine ESP (claymore & trap)", next_order(), P.get_landmine_esp, P.set_landmine_esp)
local ref_corpse_esp = make_toggle_row(visuals_world, "Corpse ESP", next_order(), P.get_corpse_esp, P.set_corpse_esp)
local set_corpse_dist_slider = make_slider(visuals_world, "Corpse max distance", next_order(), 50, 8000, P.get_corpse_esp_max_dist(), "%.0f", function(v)
	P.set_corpse_esp_max_dist(v)
end)
local ref_vehicle_esp = make_toggle_row(visuals_world, "Vehicles ESP", next_order(), P.get_vehicle_esp, P.set_vehicle_esp)
local ref_fullbright = make_toggle_row(visuals_world, "Fullbright", next_order(), P.get_fullbright, P.set_fullbright)
local ref_remove_rain = make_toggle_row(visuals_world, "Remove rain", next_order(), P.get_remove_rain, P.set_remove_rain)
...
unload_btn.Text = "Unload Project Delta"
...
task.defer(function()
	...
	set_crate_dist_slider(P.get_crate_esp_max_dist())
	set_world_dist_slider(P.get_world_esp_max_dist())
	set_corpse_dist_slider(P.get_corpse_esp_max_dist())
end)

function _G.MYA_PROJECT_DELTA_SYNC_UI()
	ref_esp()
	ref_esp_team()
	ref_esp_vis_colors()
	ref_esp_view_angle()
