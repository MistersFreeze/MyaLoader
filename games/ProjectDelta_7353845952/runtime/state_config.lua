local D = _G.MYA_PROJECT_DELTA_CONFIG or {}
...
-- Screen-space arrow showing where each enemy’s body is facing (horizontal look).
local esp_view_angle_on = D.esp_view_angle_on == true
local aim_on = false
...
-- Character model (player or NPC) for "keep on target"; nil when unused.
local aim_lock_char = nil
-- Aim assist: include NPC models (Humanoid, not a player character) in FOV pick.
local aim_assist_npcs_on = D.aim_assist_npcs_on == true
...
local npc_esp_on = D.npc_esp_on == true
local crate_esp_on = D.crate_esp_on == true
local corpse_esp_on = D.corpse_esp_on == true
local vehicle_esp_on = D.vehicle_esp_on == true
local fullbright_on = D.fullbright_on == true
local remove_rain_on = D.remove_rain_on == true
local crate_esp_max_dist = num("crate_esp_max_dist", 220, 20, 2000)
-- Corpses use a separate cap (avoids bogus map props km away).
local corpse_esp_max_dist = num("corpse_esp_max_dist", 650, 50, 8000)
local world_esp_other_max = num("world_esp_other_max", 1200, 100, 8000)
local landmine_esp_on = D.landmine_esp_on == true
local extraction_esp_on = D.extraction_esp_on == true

local gui_parent = gethui and gethui() or lp:WaitForChild("PlayerGui")
local esp_gui = Instance.new("ScreenGui")
esp_gui.Name = "MyaProjectDeltaESP"
