...
-- Single player loop when both distance + names are on (avoids duplicate Character/Head/HRP work).
local function update_esp_distance_names_combined()
	...
	for _, plr in ipairs(Players:GetPlayers()) do
		if plr ~= lp and not is_esp_teammate(plr) then
			...
			if not char or not hrp or not head or not hrp:IsA("BasePart") or not head:IsA("BasePart") then
				...
				local distStuds = (myRoot.Position - hrp.Position).Magnitude
				local posBelow, onBelow = camera:WorldToViewportPoint(hrp.Position + Vector3.new(0, -DIST_BELOW_HRP, 0))
				if onBelow and posBelow.Z > 0 then
					ensure_distance_text(plr)
					local dd = distance_draw[plr]
					dd.Position = Vector2.new(posBelow.X, posBelow.Y)
					dd.Text = string.format("%.0fm", distStuds)
					dd.Visible = true
				else
					hide_distance(plr)
				end
				local posAbove, onAbove = camera:WorldToViewportPoint(head.Position + Vector3.new(0, NAME_UP, 0))
				if onAbove and posAbove.Z > 0 then
					ensure_name_text(plr)
					local nt = name_draw[plr]
					nt.Position = Vector2.new(posAbove.X, posAbove.Y)
					local disp = plr.DisplayName
					if type(disp) == "string" and disp ~= "" then
						nt.Text = disp
					else
