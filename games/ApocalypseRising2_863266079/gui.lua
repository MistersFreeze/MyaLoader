...
if typeof(fetch) ~= "function" or typeof(repoBase) ~= "string" or #repoBase == 0 then
	error("[MyaUniversal] MYA_FETCH / MYA_REPO_BASE missing — mount via hub with ctx.baseUrl, or set init globals.")
end
local libSrc = fetch(repoBase .. "lib/mya_game_ui.lua")
if typeof(libSrc) ~= "string" or #libSrc == 0 then
	error("[MyaUniversal] Could not load lib/mya_game_ui.lua from repo base: " .. repoBase)
end
local libFn = loadstring(libSrc, "@lib/mya_game_ui")
if typeof(libFn) ~= "function" then
	error("[MyaUniversal] lib/mya_game_ui.lua failed to compile")
end
local MyaUI = libFn()
...
local ref_arrows_esp = make_toggle_row(
	visuals_esp,
	"Crosshair arrows (point at enemies)",
	next_order(),
	P.get_arrows_esp,
	P.set_arrows_esp
)
local set_arrows_ring_slider = make_slider(
	visuals_esp,
	"Arrow ring radius (px)",
	next_order(),
	32,
	220,
	P.get_arrows_esp_ring_radius(),
	"%.0f",
	function(v)
		P.set_arrows_esp_ring_radius(v)
	end
)
local ref_arrows_distance = make_toggle_row(
	visuals_esp,
	"Distance under arrows",
	next_order(),
	P.get_arrows_esp_distance,
	P.set_arrows_esp_distance
)
