...
function M.mount(ctx)
	...
	local function fetch(u)
		...
		if root and typeof(readfile) == "function" then
			...
			if string.sub(nu, 1, #nr) == nr then
				...
				error("Booga Booga: readfile failed: " .. u)
			end
		end
	...
	local runChunk = loadstring(runSrc, "@BoogaBooga/runtime")
	if typeof(runChunk) ~= "function" then
		error("Booga Booga: runtime.lua failed to compile")
	end
	local runLoader = runChunk()
	if typeof(runLoader) ~= "function" then
		error("Booga Booga: runtime.lua must return a bundle loader")
	end
	runLoader({ base = base, fetch = fetch })

	local guiSrc = fetch(base .. "gui.lua")
	local guiChunk = loadstring(guiSrc, "@BoogaBooga/gui")
	if typeof(guiChunk) ~= "function" then
		error("Booga Booga: gui.lua failed to compile")
	end
	guiChunk()
	...
	ctx.notify("Booga Booga loaded")
end

function M.unmount()
	if typeof(_G.unload_mya_booga) == "function" then
		pcall(_G.unload_mya_booga)
	end
end
