--[[ Flex Your FPS And Ping — PlaceId 18667984660 ]]
...
function M.mount(ctx)
	...
	local function fetch(u)
		...
		if root and typeof(readfile) == "function" then
			...
			if string.sub(nu, 1, #nr) == nr then
				...
				error("Flex Your FPS: readfile failed: " .. u)
			end
		end
	...
	local runChunk = loadstring(runSrc, "@FlexYourFPS/runtime")
	if typeof(runChunk) ~= "function" then
		error("Flex Your FPS: runtime.lua failed to compile")
	end
	local runLoader = runChunk()
	if typeof(runLoader) ~= "function" then
		error("Flex Your FPS: runtime.lua must return the bundle loader")
	end
	runLoader({ base = base, fetch = fetch })

	local guiSrc = fetch(base .. "gui.lua")
	local guiChunk = loadstring(guiSrc, "@FlexYourFPS/gui")
	if typeof(guiChunk) ~= "function" then
		error("Flex Your FPS: gui.lua failed to compile")
	end
	guiChunk()

	if typeof(_G.MYA_FLEX_RUN_UI_SYNC) == "function" then
		_G.MYA_FLEX_RUN_UI_SYNC()
	end

	ctx.notify("Flex Your FPS loaded")
end
