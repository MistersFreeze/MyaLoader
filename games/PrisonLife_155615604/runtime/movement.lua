...
--- VehicleSeat model root for Prison Life / driving games.
local function get_car_fly_root_part()
...
local function start_car_fly()
	...
	car_fly_conn = RunService.RenderStepped:Connect(function()
		...
		local root = get_car_fly_root_part()
		if not root then
			if car_fly_bv then
				pcall(function()
					car_fly_bv:Destroy()
		...
		-- Ground-plane directions from camera view (W = straight ahead where you look, no “flying up” from pitch).
		local look = cam.CFrame.LookVector
		local flatFwd = Vector3.new(look.X, 0, look.Z)
		if flatFwd.Magnitude < 1e-4 then
			flatFwd = Vector3.new(0, 0, -1)
		else
			flatFwd = flatFwd.Unit
		end
		local flatRight = Vector3.new(0, 1, 0):Cross(flatFwd).Unit
		local dir = Vector3.zero
		if UserInputService:IsKeyDown(Enum.KeyCode.W) then
			dir = dir + flatFwd
		end
